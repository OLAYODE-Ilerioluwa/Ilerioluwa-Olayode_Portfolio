...
with service ID `ilerioluwa-olayodeportfolio`
hosted in `us-east4`.
...
is the query named "ReadData",
which can be accessed via the [ExampleConnector.readData] property.


An example of the property for a mutation
is the mutation named "CreateData",
which can be accessed via the [ExampleConnector.createData] property.
...
For example, the "ReadData" query has no variables
and can be executed via the
[ExampleConnector.readData]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.readData.execute()
println("ReadData query returned: ${queryResult.data}")
```
...
For example, the "CreateData" mutation has no variables
and can be executed via the
[ExampleConnector.createData]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createData.execute()
println("CreateData mutation returned: ${mutationResult.data}")
```
